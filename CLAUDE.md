...
- **CI**: the `e2e` job lives in `.github/workflows/ci.yml` and runs exactly what a dev machine runs — full app on the runner, throwaway Postgres, no Neon and no deployed preview (that switch removed serverless cold starts and Neon round-trip latency as flake sources). It needs `BETTER_AUTH_SECRET`, `HOMEWISE_RESEND_API_KEY` and `HOMEWISE_FILES_READ_WRITE_TOKEN`; the blob token must be real because the photo specs upload for real. `DATABASE_URL` is **not** set on the job — `globalSetup`/`webServer` supply the test DB. Neon still backs the *deployed* preview (`preview.yml` migrates + seeds a per-PR branch during the server build), but no test touches it.
...
- **A new mutating endpoint isn't done until it emits.** Add `c.var.emit(...)` in the handler and map the entity in the web's `invalidators` record. Nothing fails loudly if you skip it — the only symptom is a second member's browser quietly showing stale data. See the realtime sections above.
