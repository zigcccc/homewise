...
Also derived from the column, so don't restate it: nullability, enum values, and `numeric` range bounds. **Not** derived, so you must state it: a `date` column generates a bare `z.string()` that accepts `2026-13-45` and `nope` alike, and `numeric(12,2)` generates neither a decimal check nor a positivity one — money always keeps an explicit `moneyAmount(...)`.
...
**And an `errorComponent`, scoped to that route.** Without one, a loader rejection — or a realtime refetch that lands on a 404 because another member deleted the thing — climbs to the root boundary and replaces the *entire app*, sidebar included, with "Something went wrong!". Render an `Empty` in the route's own pane saying what's missing and offering the way out (`shopping-lists/$listId.tsx` is the model). A layout route covers the children rendering into its `<Outlet />`, so an overlay route doesn't need its own. Where a subject can genuinely vanish, say so specifically ("This list is gone"); where it can't, a generic "couldn't load" plus a retry is honest.
