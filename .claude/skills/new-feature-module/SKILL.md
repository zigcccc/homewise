...
> externals and pets are denied it unless you say otherwise. A new *event entity* also needs its area
...
`pnpm check-types`, then `pnpm lint` to **zero diagnostics**, then `pnpm knip`, then `pnpm test`. All
four, before the E2E gate: knip is the only one that flags a dependency a `package.json` declares but
nothing in that package imports, and the unit suite is the only one that runs the code. Type-checking
green is not evidence the feature works — that's what the E2E flow below is for.
