...
frame rather than describing it twice: a `DashboardShell` for the page, one `const CARD = { … }` per
card feeding both the card and its exported `…Skeleton` (see
`_authenticated/_onboarded/-components/`).
