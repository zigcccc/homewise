 ...
 */
export const Route = createFileRoute('/_authenticated/_onboarded/external')({
  beforeLoad({ context }) {
    if (context.role !== 'external') {
      throw redirect({ to: '/' });
    }
  },
  component: ExternalHomeRoute,
  pendingComponent: ExternalHomePending,
...
function ExternalShell({ children, header }: { children: React.ReactNode; header: React.ReactNode }) {
  return (
    <>
      <Actionbar.Content>
        <Breadcrumb>
    ...
    <ExternalShell
      header={
        <div>
          ...
          <p className="text-muted-foreground text-sm" data-testid="external-greeting">
